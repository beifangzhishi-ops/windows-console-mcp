...
The approval View uses the official `@modelcontextprotocol/ext-apps` `App` client. Vite builds it as one self-contained HTML file. At router startup WCM hashes that HTML and derives the active `ui://wcm/approval-test/<hash>.html` resource URI, so changed UI content automatically gets a fresh host cache key. The approval-card tool descriptor publishes the standard MCP Apps `ui.resourceUri`/`ui.visibility` fields together with the ChatGPT host fields `ui/resourceUri`, `openai/outputTemplate`, and `openai/widgetAccessible`; the resolver remains app-only and widget-accessible.
